...
def menu():
    clear()
    print(f'{R}❲{A}0{R}❳{G} FACEBOOK  HACK > 500 ')
    print(f'{R}❲{A}0{R}❳{G} FREE FIRE  HACK > 500') 
    print(f'{R}❲{A}00{R}❳{G} GMAIL HACK >500');linex()
    option=input(f'{R}❲{A}?{R}❳{G} PEYMENT AND USE  {R}:{G} ')
    if option in ['1','A']:
        filex()
    elif option in ['2','B']:
    	os.system('xdg-open https://www.book.com/sk.sahathat');menu()
    else:
